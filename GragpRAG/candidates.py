...
모든 재료의 `usage_count` 가 0 이 되어 `ORDER BY` 가 근거를 잃었다.

10회 실행해 보면 결과는 **일정하다.** 무작위가 아니라 저장 순서로 고정된다.
그래서 더 나쁘다 — 흔들렸으면 드러났을 텐데, 안정적으로 질문과 무관한 답을 준다.
이것이 재료 경로 후보 밖 생성률 50% 의 원인이다.

실측 (2025-05-03 데이터 1회 적재, Ingredient 98개 — `eval_candidates.py`):
    예전: 밀가루, 버터, 크림, 빵, 그릭 요거트 ...   (정렬 키 전부 0)
    지금: 딸기(209), 크림(208), 초콜릿(183) ...     (빈도 합 기준)

`count` 는 누적되므로 2회 적재하면 freq 가 2배가 된다. 측정 시 회차 수를 확인할 것.
