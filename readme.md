...
결론부터: 저장된 답변에서 메뉴 경로는 지켜졌고 재료 경로는 지켜지지 않았다.
**그 실패의 원인으로 지목했던 Cypher 버그는 A/B 로 반증됐다** — 후보만 바꿔 다시 재니
양쪽 팔 모두 위반이 0건이었다. 버그는 실재하고 고쳤지만, 환각의 원인이라는 근거는 없다.
...
| 후보 밖 생성률 — 재료 경로 | **50.0%** (10/20) | 같음. 단 답변 4건 중 2건이 전량 위반 |
| └ A/B 재측정 (후보만 교체) | old·new **둘 다 0%** | gpt-4 30답변 + gpt-4o-mini 24답변 |
...
### A/B 재측정 — 인과 주장은 서지 않는다

"후보가 나빠서 샜다"를 검증하려면 **후보만 바꿔** 봐야 한다.
같은 모델·프롬프트·질문·반복 수에서 후보 목록만 교체했다.

```bash
python GragpRAG/eval_grounding_ab.py --repeat 4 --model gpt-4
python GragpRAG/eval_grounding_ab.py --historical   # 저장된 답변을 답변 단위로 분해
```

| 조건 | 모델 | 항목 | 후보 밖 |
|---|---|---|---|
| 재료 old / new | gpt-4o-mini | 20 / 20 | **0 / 0** |
| 재료 old / new | **gpt-4** | 20 / 20 | **0 / 0** |
| 재료 old / new (5적재 × 3회) | **gpt-4** | 72 / 75 | **0 / 0** |
| 메뉴 old / new (통제군) | gpt-4 | 100 / 99 | 0 / 1 |

**엉터리 후보를 줘도 벗어나지 않는다.** 게다가 역사적 50% 를 답변 단위로 풀면
**같은 후보 목록이 100% 와 0% 를 둘 다 냈다** (10차 5/5, 2025-04-30 0/5).
후보가 원인이라면 같은 후보에서 같은 결과가 나와야 한다. N 이 답변 4건이다.

**그래도 수정은 유지한다.** 환각과 별개로 옛 쿼리가 틀렸기 때문이다 —
순위에 근거가 없고, 없는 관계를 조회하며, **적재할 때마다 결과가 바뀐다**
(5회 적재가 3가지 목록을 냈다).
...
- ~~재료 경로 후보 밖 생성률 50%~~ → 후보 Cypher 수정 + A/B 재측정 완료.
  **재현 조건을 모른다는 것이 남았다.** 새 측정 54개 답변에서 위반이 한 건도 안 나왔고,
  역사적 2건이 어떤 조건에서 나왔는지 특정하지 못했다 — 모델 스냅샷 변경, 그때의 그래프
  상태, temperature 분산 중 무엇인지 가릴 데이터가 없다. 런타임 격리는 동작하므로
  유출은 막히지만, 이 실패 모드는 **감시 대상**으로만 남는다.
