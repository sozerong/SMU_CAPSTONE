...
`db.relationshipTypes()` 결과에도 없다 (`HAS_CATEGORY, HAS_EXAMPLE, HAS_SEASON,
HAS_TAG, INCLUDES, IS_COMBO_WITH, RECORDED_ON, RELATED` 뿐).
`OPTIONAL MATCH` 라 에러는 나지 않고, **모든 재료의 `usage_count` 가 0** 이 되어
전부 동점이 된다.

동점일 때 어떻게 되는지는 재 봐야 안다. 10회 실행 결과:
...
**실행마다 바뀌지 않는다.** 저장 순서로 결정되고 그 순서는 안정적이다.
"정렬이 무작위가 된다"는 부정확하고, 정확히는 **순위에 근거가 없다**는 것이다.
결과는 "유행하는 재료 10개"가 아니라 "저장 순서상 앞의 10개"다.

안정적으로 엉뚱한 답을 주기 때문에 더 나쁘다. 흔들렸다면 금방 눈치챘을 것이다.
"최근 유행하는 재료"를 물었는데 후보가 질문과 무관하니 모델이 밖에서 끌어온다.
...
후보에서 빠진 것: `밀가루`·`버터`·`과일`·`견과류`·`그릭 요거트`.
앞의 넷은 어떤 `Combo` 에도 속하지 않는 일반 재료다.

**적재 횟수가 측정값을 바꾼다.** `count` 가 누적되므로 2회 적재하면 `freq` 가 2배가 된다
(딸기 209 → 418 실측). 측정 조건에 회차 수를 반드시 적는다.
